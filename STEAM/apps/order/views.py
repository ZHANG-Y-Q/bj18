...
class OrderPlaceView(LoginRequiredMixin,View):
    ...
    def post(self, request):
        ...
        total_count = 0
        total_price = 0
        # 遍历sku_ids获取用户需要购买的商品信息
        for sku_id in sku_ids:
            # 根据商品id获取商品的信息
            sku = GoodsSKU.objects.get(id=sku_id)
            # 获取用户所需要购买的商品的数量
            sku_count = request.POST.get('show_count')
            if sku_count:
                count = sku_count
            else:
